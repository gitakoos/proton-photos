...
interface PersonDao {
    ...
    /** Re-keys every person from one owner to another, to adopt a guest's people on sign-in. The
     *  autogenerated person id is global, so face/cluster references stay valid across the re-key. */
    @Query("UPDATE person SET userId = :to WHERE userId = :from")
    suspend fun updateUserId(from: String, to: String)

    /** How many people an owner holds, so a sign-in migration can report what it carried or dropped. */
    @Query("SELECT COUNT(*) FROM person WHERE userId = :userId")
