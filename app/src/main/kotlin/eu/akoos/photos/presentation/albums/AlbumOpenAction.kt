 ...
 *
 * The Albums grid holds a cover and a count per album, never its members, so the three actions that
...
object AlbumOpenIntent {
     ...
     *
     * Every action reads the album's members: two of them act on the photos, and the third hands
     * them over as the picker's exclude set, so all three wait for [photosLoaded] rather than firing
     * against the empty list the screen mounts with. Past that point they part ways on [photoCount]:
     * adding is precisely what an empty album is opened for, while downloading or playing nothing is
     * a confirmation with no answer behind it. [alreadyRun] is the within-composition half of the
    ...
    fun shouldRun(
        ...
        return when (action) {
            ...
            AlbumOpenAction.DownloadAll, AlbumOpenAction.Slideshow -> photoCount > 0
        }
    }
}
